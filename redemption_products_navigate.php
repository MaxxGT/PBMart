...
if($last != 1){
	...
	{
        $previous = $pagenum - 1;
		$paginationCtrls .= "<a href='redemption.php?hyperlink=redemption&id=$class&pg=$previous&product_categories=$product_categories&keyword=$keyword'> Previous | </a>";
		// Render clickable number links that should appear on the left of the target page number
		for($i = $pagenum-4; $i < $pagenum; $i++){
			if($i > 0){
		        $paginationCtrls .= "<a href='redemption.php?hyperlink=redemption&id=$class&pg=$i&product_categories=$product_categories&keyword=$keyword'> $i |</a>";
			}
	    }
    }
	...
	for($i = $pagenum+1; $i <= $last; $i++){
		$paginationCtrls .= "<a href='redemption.php?hyperlink=redemption&id=$class&pg=$i&product_categories=$product_categories&keyword=$keyword' title='Go To Page $i'> $i |</a>";
		if($i >= $pagenum+4){
			break;
		}
	}
    ...
    if ($pagenum != $last) {
        $next = $pagenum + 1;
        $paginationCtrls .= "<a href='redemption.php?hyperlink=redemption&id=$class&pg=$next&product_categories=$product_categories&keyword=$keyword' title='Next Page'> Next</a>";
    }else
	{
