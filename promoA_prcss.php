...
if($btnRd_product == '0')
{
	echo $sql_promoA = "SELECT * FROM pbmart_promotion WHERE promotion_category_id = '1' AND promotion_show = '1' AND promotion_package_stock !='0' AND promotion_package_stock >'0' AND promotion_start_date <='$current_date' AND promotion_end_date >='$current_date'";
}else if($btnRd_product == '1')
{
	echo $sql_promoA = "SELECT * FROM pbmart_promotion WHERE promotion_category_id = '1' AND promotion_show = '0' AND promotion_package_stock !='0' AND promotion_package_stock >'0' AND promotion_start_date <='$current_date' AND promotion_end_date >='$current_date'";
}else
{
	echo $sql_promoA = "SELECT * FROM pbmart_promotion WHERE promotion_category_id = '1' AND promotion_show = '1' AND promotion_package_stock !='0' AND promotion_package_stock >'0' AND promotion_start_date <='$current_date' AND promotion_end_date >='$current_date'";
}
