...
if(isset($class) && isset($redeem_class))
{
	...
	}else
	{
		...
		$filter= "WHERE redeem_stock!='0' AND redeem_class='$redeem_class'";
						...
						$sql_products = "SELECT * FROM pbmart_redeem $filter";
...
if($last != 1){
	...
	{
        $previous = $pagenum - 1;
		$paginationCtrls .= "<a href='redemption.php?id=$class&hyperlink=redemption&pg=$previous&product_categories=$product_categories&keyword=$keyword'> Previous | </a>";
		// Render clickable number links that should appear on the left of the target page number
		for($i = $pagenum-4; $i < $pagenum; $i++){
			if($i > 0){
		        $paginationCtrls .= "<a href='redemption.php?id=$class&hyperlink=redemption&pg=$i&product_categories=$product_categories&keyword=$keyword'> $i |</a>";
			}
	    }
    }
	...
	for($i = $pagenum+1; $i <= $last; $i++){
		$paginationCtrls .= "<a href='redemption.php?id=$class&hyperlink=redemption&pg=$i&product_categories=$product_categories&keyword=$keyword' title='Go To Page $i'> $i |</a>";
		if($i >= $pagenum+4){
			break;
		}
	}
    ...
    if ($pagenum != $last) {
        $next = $pagenum + 1;
        $paginationCtrls .= "<a href='redemption.php?id=$class&hyperlink=redemption&pg=$next&product_categories=$product_categories&keyword=$keyword' title='Next Page'> Next</a>";
    }else
	{
