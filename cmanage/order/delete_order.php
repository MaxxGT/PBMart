		...
		if(!isset($_POST['orderList'])){
			...
			for($i = 0; $i < $count; $i++){
				...
				if($id > 0){
					...
					$com_date = mysqli_real_escape_string($dbconnect, $_POST['complete_date']);
					
					$complete = mysqli_query($dbconnect, "UPDATE pbmart_order SET order_payment_status='1', order_status='1', order_complete_date='$com_date' WHERE order_number='".$order_display['order_number']."'");
					
					$salesman_id = $_POST['delivery_salesman'];
					$upd_dlv_slms = mysqli_query($dbconnect, "UPDATE pbmart_member SET salesman_id='".$salesman_id."' WHERE member_id='".$order_display['order_customer_id']."'");
