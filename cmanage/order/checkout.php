...
//last update: 06-10-2016 2:38PM
	...
	if(isset($_POST['checkout'])){
		...
		$total_mygaz = mysqli_real_escape_string($dbconnect, $_POST['total_mygaz']);
		$total_petronas = mysqli_real_escape_string($dbconnect, $_POST['total_petronas']);
		...
		if($firstName == "" || $lastName == "" || $streetName == "" || $postcode == "" || $city == "" || $state == "" || $country == "" || $delivery == ""){
			...
			$create_order = mysqli_query($dbconnect, "INSERT INTO pbmart_order(order_number, order_amount, order_handling, order_date, order_delivery, order_time, order_time_date, order_customer_id, order_customer_name, order_customer_contact, order_customer_address, order_payment_type, order_payment_status, order_status, order_mygaz_amount, order_petronas_amount) VALUES ('$order_num', '$total_amount', '$total_handling', '$date', '$delivery', '$pre_time', '$order_time', '$id', '$name', '$contact', '$address', 'Cash', '0', '0', '$total_mygaz', '$total_petronas')");
			...
			if($cart_display['cart_product_class'] == "Product"){
				...
				if($member_display['member_commercial_status'] == 1){
					if($member_display['member_commercial_class'] == 1){
						if($pro_display['product_commercial_double_point'] == 1){
							...
							$pro_point = (((($pro_display['product_commercial_point']+$commercial_display['commercial_additional_point']) * $total_mygaz) + ($pro_display['product_commercial_point_ptrs']+$commercial_display['commercial_additional_point']) * $total_petronas) * 3);
							
							$prd_point = ($pro_display['product_commercial_point'] + $commercial_display['commercial_additional_point']) * 3;
						}else{
							if($pro_id =='1' || $pro_id =='3')
							{
								$pro_point = (($pro_display['product_commercial_point'] + $commercial_display['commercial_additional_point']) * $total_mygaz) +(($pro_display['product_commercial_point_ptrs'] + $commercial_display['commercial_additional_point']) * $total_petronas) ;
								$prd_point = $pro_display['product_commercial_point'] + $commercial_display['commercial_additional_point'];
							}else
							{
						...
						if($pro_display['product_commercial_double_point2'] == 1){
							...
							$pro_point = ((($pro_display['product_commercial_point2'] + $commercial_display['commercial_additional_point']) * $total_mygaz) + (($pro_display['product_commercial_point2_ptrs'] + $commercial_display['commercial_additional_point']) * $total_petronas)) * 3;
							
							$prd_point = ($pro_display['product_commercial_point2'] + $commercial_display['commercial_additional_point']) * 3;
						}else{
							if($pro_id == '1' || $pro_id =='3')
							{
								$pro_point = (($pro_display['product_commercial_point2'] + $commercial_display['commercial_additional_point']) * $total_mygaz) + (($pro_display['product_commercial_point2_ptrs'] + $commercial_display['commercial_additional_point']) * $total_petronas);
								$prd_point = $pro_display['product_commercial_point2'] + $commercial_display['commercial_additional_point'];
							}else
							{
				...
				}else
				{
					...
					if($pro_display['product_double_point'] == 1){
						$pro_point = (($pro_display['product_point'] * $total_mygaz) + ($pro_display['product_point_ptrs'] * $total_petronas)) * 3;
						$prd_point = $pro_display['product_point'] * 3;
					}else{
						//$pro_point = $pro_display['product_point'];
						if($pro_id =='1' || $pro_id =='3')
						{
							$pro_point = ($pro_display['product_point'] * $total_mygaz) + ($pro_display['product_point_ptrs'] * $total_petronas);
							$prd_point = $pro_display['product_point'];
							if($total_mygaz=='0' && $total_petronas=='0')
							{
								$pro_point = $pro_display['product_point'];
								$prd_point = $pro_display['product_point'];
							}
				...
				$pro_mod = mysqli_query($dbconnect, "SELECT * FROM pbmart_product WHERE product_id='$pro_id'");
				$pro_model = mysqli_fetch_assoc($pro_mod);
				
				$create_order_list = mysqli_query($dbconnect, "INSERT INTO pbmart_order_list(order_number, order_product_id, order_product_class, order_product_name, order_product_model, order_product_price, order_product_handling, order_product_point, order_product_sale, order_product_amount) VALUES ('$order_num', '$pro_id', 'Product', '$pro_name', '".$pro_model['product_model']."', '$pro_price', '$pro_handling', '$prd_point', '$pro_sale', '$pro_amount')");
			
				if($pro_id =='1' || $pro_id =='3')
				{
					$total_point = $total_point + ($pro_point);
				}else
				{
