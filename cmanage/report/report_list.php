...
if(isset($_POST['btnflt']))
{
	...
	}else if($btnflt == 'Domestic')
	{
		$sql = "Select * FROM pbmart_order
						INNER JOIN pbmart_member ON
						pbmart_member.member_id = pbmart_order.order_customer_id
						WHERE 
								MONTH(pbmart_order.order_complete_date) BETWEEN '$month_from' AND '$month_to' 
								AND YEAR(pbmart_order.order_complete_date) = '$YEARS' 
								AND pbmart_order.order_status='1'
								AND pbmart_member.member_status='1'
								AND pbmart_member.member_commercial_status ='0'";
	}else if($btnflt == 'Commercial')
	{
		$sql = "Select * FROM pbmart_order
						INNER JOIN pbmart_member ON
						pbmart_member.member_id = pbmart_order.order_customer_id
						WHERE 
								...
								AND pbmart_order.order_status='1'
								AND pbmart_member.member_status='1'
								AND pbmart_member.member_commercial_status ='1'
								AND pbmart_member.member_commercial_class ='1'";
	}else if($btnflt == 'Commercial(K)')
	{
		$sql = "Select * FROM pbmart_order
						INNER JOIN pbmart_member ON
						pbmart_member.member_id = pbmart_order.order_customer_id
	...
	<div class="grid_16" id="content">	
		...
		<form name='rprt_lst_form' action='report_list.php' method='post'>	
			<table class="box-table-a" align="center"width="928px">
				...
				<tr>
					<td colspan="4">
						<strong>MEMBERS: <?php echo $btnflt; ?></strong>
						<BR />
						<strong>FILTER MEMBERS: </strong>
						<input type='submit' name='btnflt' value='ALL' />
						<input type='submit' name='btnflt' value='Domestic' />
						<input type='submit' name='btnflt' value='Commercial' />
						<input type='submit' name='btnflt' value='Commercial(K)' />
					</td>
				</tr>
				<tr>
						<th width='20px'>No.</th>
				...
				?>
					<tr>
						...
						<td><?php echo $order_complete_date; ?></td>
