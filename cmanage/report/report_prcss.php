...
<?php 
	...
	<?php
	}else if($btnSubmit == "Generate Products PDF")
	{ ?>
		<form id="form" name="form" action="report_product_pdf.php" method="post">
			<?php
				if(isset($product) && $product !="")
				{
					foreach($product as $member)
	...
	<?php }
	else if($btnSubmit == "Generate Promotions PDF")
	{ ?>
		<form id="form" name="form" action="report_promotion_pdf.php" method="post">
			<?php
				if(isset($promotions) && $promotions !="")
				{	
					foreach($promotions as $member)
			...
			<input type="hidden" name="month_to" value="<?php echo $month_to; ?>"></input>
			<script language="JavaScript">document.form.submit();</script>
			<script language='JavaScript'>window.top.location ="sale_report.php"</script>
		</form>
	<?php	
	}else if($btnSubmit == "Generate Flat Handling Reports")
	{ ?>
		<form id="form" name="form" action="report_flat_handling.php" method="post">
			<?php
				if(isset($promotions) && $promotions !="")
				{	
					foreach($promotions as $member)
	...
	<?php	
	}else if($btnSubmit == "Generate Flat Handling PDF")
	{ ?>
		<form id="form" name="form" action="report_flat_handling_pdf.php" method="post">
			<?php
				if(isset($promotions) && $promotions !="")
				{	
					foreach($promotions as $member)
