			...
			<div id="tabs">
				 <div class="container">
					<ul>
						...
						<li><a href="../order_manage.php" target='_blank'><span>Order Manage</span></a></li>
					...
					<table border="1" align="center" width="900px" class="box-table-a" cellpadding="0" cellspacing="0">
						...
						<tr>
							<tr>
								...
								<th width="150px" ststyle="padding-left:5;">Name</th>
								<th width="60px" style="padding-left:5;">Points</th>
						...
						<?php	while($order_display = mysqli_fetch_array($order)){	?>
							<tr class="link" href="order/view_orderList.php?or=<?=$order_display['order_number']?>&view=ma&hyperlink=orders">
								...
								<?php	$commercial = mysqli_query($dbconnect, "SELECT * FROM pbmart_member WHERE member_id='".$order_display['order_customer_id']."'");
										$commercial_status = mysqli_fetch_assoc($commercial);
										
										if($commercial_status['member_commercial_status'] == 0){
											$name = $order_display['order_customer_name'];
										}else{
											$com_name = mysqli_query($dbconnect, "SELECT * FROM pbmart_commercial WHERE commercial_member_id='".$order_display['order_customer_id']."'");
											$com_name_display = mysqli_fetch_assoc($com_name);
											
											$name = $com_name_display['commercial_company'];
										}
								?>
								<td width="150px" style="padding-left:5;"><?=$name?></td>
								<td width="60px" style="padding-left:5;"><?=$order_display['order_total_point']?></td>
